...
const AdminTeamOverview = () => {
  ...
  const { user: users, loading: userLoading } = useAppSelector(
  ...
  const usersInTeams = users?.filter((user: User) => user.team).length || 0;
  ...
  const totalTeams = new Set(
    users
      ...
      .map((user: any) => user.team.toString())
  ).size;
  ...
  const getProgress = (label: string) => {
    ...
    if (label === "Users not in Teams")
      return (usersNotInTeams / users.length) * 100;
    return 100;
  };
    ...
    <Box p={4}>
      ...
      `}
          ...
          <div className="bg-white p-4 rounded-lg shadow-md w-full md:w-[600px] lg:w-[700px]">
            ...
            {notificationError && (
              <div className="text-red-500">{notificationError}</div>
            )}
