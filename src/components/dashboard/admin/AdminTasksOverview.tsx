...
const AdminTaskOverview = () => {
  ...
  const {
    ...
    loading: userLoading,
    error,
  } = useAppSelector((state) => state.createUser);
  ...
  useEffect(() => {
    if (users && users.length > 0) {
      const imagePromises = users.map((user: User) => {
        if (user.image) {
  ...
  const memberStats: Record<
    string,
    { completed: number; incomplete: number; image?: string }
  > = {};

  tasks.forEach((task) => {
    const member = task.assignedTo?.name || "Unknown";
    const image = users?.find((user: User) => user.name === member)?.image;
  ...
  const getProgress = (label: string) => {
    ...
    if (label === "Incomplete Tasks")
      return (incompleteTasks / tasks.length) * 100 || 0;
    return 100;
  };
          ...
          <div className="flex flex-col md:flex-row justify-between gap-10 mb-10">
            ...
            <div className="flex flex-wrap gap-10 md:w-2/3 justify-center">
              {stats.map((stat, index) => (
                <div
                  key={stat.label}
                  className={`min-w-[200px] flex justify-center items-center h-[160px] ${
                    index >= 2 ? "w-full md:w-[200px]" : "w-full md:w-[200px]"
                  }`}
                >
              ...
              <Box className="bg-white p-4 rounded-lg shadow-md mt-10 w-full md:w-[800px]">
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={[...sortedCompleted, ...sortedIncomplete]}>
                    ...
                    <Bar
                      dataKey="incomplete"
                      fill={dashboardColors.notCompleted}
                    />
                  </BarChart>
                </ResponsiveContainer>
              </Box>
